function [ret_density, ret_velocity, ret_pressure, is_left_of_contact, iteration] = ExacRiemanSolver(left_density, left_velocity, left_pressure, right_density, right_velocity, right_pressure, Gamma, lambda, MaxIteration, TOL)
...
if (critical_speed < 0.0) % образуется зона вакуума
    l
    eft_head_speed = left_velocity - left_soundspeed;
...
if (is_left_of_contact) % определяем, где находится искомая линия lambda,
    ...
    if (p_star > pMax)
        ...
        if (p_star < pMin)
            ...
            p_star = ((left_soundspeed + right_soundspeed + 0.5 * (Gamma - 1.0) * (left_velocity - right_velocity)) / (left_soundspeed / left_pressure^temp1 + right_soundspeed / right_pressure^temp1))^(1.0 / temp1);
